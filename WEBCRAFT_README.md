...
To make a server for WebCraft 1.8 the recommended software to use is EaglerXServer ("EaglerXServer"), which you can get from Mutant Bloodcell here: [https://github.com/MutantBloodcell/eaglerxserver/](https://github.com/MutantBloodcell/eaglerxserver/)

## Launch Options

The WebCraft 1.8 client is configured primarily through a variable called `window.eaglercraftXOpts` that must be set before the client starts up.
...
The default eaglercraftXOpts values is this:
    ...
    window.eaglercraftXOpts = {
        ...
        relays: [
            { addr: "wss://relay.deev.is/", comment: "mutant relay #1", primary: relayId == 0 },
            { addr: "wss://relay.lax1dude.net/", comment: "mutant relay #2", primary: relayId == 1 },
            { addr: "wss://relay.shhnowisnottheti.me/", comment: "web relay #1", primary: relayId == 2 }
        ]
    };
...
- `eaglerNoDelay:` can be used to disable "Vigg's Algorithm", an algorithm that delays and combines multiple WebCraft packets together if they are sent in the same tick (does not affect regular Minecraft 1.8 packets)
